...
impl Commands {
        ...
        W: Write,
    {
        ...
        if self.verbose {
            writeln!(writer, "{verbose_msg}: ${price:.6}")?;
            ...
            writeln!(writer, "{price:.6}")?;
        }
        Ok(())
    }

    pub async fn process_account_list<W: ?Sized>(
        &self,
        db: &Db,
        ...
        W: Write,
    {
        ...
        if accounts.is_empty() {
            ...
            if !disposed_lots.is_empty() {
                writeln!(writer, "Disposed ({} lots):", disposed_lots.len())?;
            ...
            writeln!(writer, "Realized Gains")?;
            if unified_tax_rate {
                writeln!(
                    writer,
                    "  Year    | Income          |       Cap gain | Estimated Tax "
                )?;
            } else {
                writeln!(
                    writer,
            ...
            writeln!(writer, "Current Holdings")?;
            ...
            if pending_deposits > 0 {
                writeln!(writer, "  !! Pending deposits: {pending_deposits}")?;
            }
            if pending_withdrawals > 0 {
                writeln!(writer, "  !! Pending withdrawals: {pending_withdrawals}")?;
            }
            if pending_transfers > 0 {
                writeln!(writer, "  !! Pending transfers: {pending_transfers}")?;
            }
            if pending_swaps > 0 {
                writeln!(writer, "  !! Pending swaps: {pending_swaps}")?;
            }
        }
...
async fn maybe_println_lot<W: ?Sized>(
    token: MaybeToken,
    lot: &Lot,
